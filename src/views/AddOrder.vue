...
export default {
  ...
  created(){
    ...
    axios.get('http://localhost:8181/user/findBylogin_name/'+login_name).then(function (resp){
      _this.ruleForm.userid = resp.data.id;
    })
  },
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios.post('http://localhost:8181/order/save',this.ruleForm).then(function (resp){
            if(resp.data == 'success'){
              _this.$alert('订单创建成功', '消息', {
